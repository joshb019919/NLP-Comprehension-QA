...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run test only and overwrite final test outputs.")
...
def overwrite_test_outputs(output_dir: Path, test_metrics: dict[str, float]) -> None:
    test_results_path = output_dir / "test_results.json"
    all_results_path = output_dir / "all_results.json"
    summary_metrics_path = output_dir / "summary_metrics.json"

    atomic_save_json(test_metrics, test_results_path)

    all_results = load_json_if_exists(all_results_path)
    all_results = {key: value for key, value in all_results.items() if not key.startswith("test_")}
    ...
    summary_metrics["test_metrics"] = test_metrics
...
def main() -> None:
    ...
    if config.dataset.test_split is None:
        raise ValueError("This run configuration has no test split configured.")
    ...
    try:
        ...
        raw_test, test_examples, test_features_for_postprocess, tokenizer, data_collator = build_qa_split(
        ...
        elif test_dataset_name in {"rajpurkar/squad", "rajpurkar/squad_v2"}:
            test_features_for_predict = test_features_for_postprocess.remove_columns(["example_id", "offset_mapping"])
        else:
            raise ValueError(f"Unsupported dataset_name={test_dataset_name!r}")
        ...
        trainer = QATrainer(
            ...
            args=training_args,
            train_dataset=None,
            eval_dataset=test_features_for_predict,
            data_collator=data_collator,
            dataset_name=test_dataset_name,
            version_2_with_negative=test_version_2_with_negative,
            sgd_momentum=config.run.sgd_momentum,
            raw_eval_examples=None,
            postprocess_eval_examples=None,
            postprocess_eval_features=None,
            callbacks=[],
        )
        ...
        overwrite_test_outputs(output_dir, test_metrics)
