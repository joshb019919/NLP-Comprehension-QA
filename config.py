...
class DatasetConfig(Config):
    ...
    validation_dataset_config_name: str | None = None
    validation_version_2_with_negative: bool | None = None
    test_split: str | None = "test"
    test_dataset_name: str | None = None
    test_dataset_config_name: str | None = None
    test_version_2_with_negative: bool | None = None
...
class RunConfig(Config):
    ...
    # Useful for quick debugging without changing raw-example limits.
    limit_after_tokenization: bool = True
    after_tokenization_limit: int | None = None
    after_tokenization_train_limit: int | None = 20000
    after_tokenization_validation_limit: int | None = 5000
    after_tokenization_test_limit: int | None = 5000

    # Optimization
    learning_rate: float = 5e-5
    # batch_size: int = 8  # Set for easier VRAM
    batch_size: int = 64   # Set for more VRAM use
    epochs: int = 2
    lr_scheduler: str = "cosine"
    weight_decay: float = 1e-8
    # gradient_accumulation_steps: int = 8    # Set for high it, high it/s
    gradient_accumulation_steps: int = 1      # Set for low it, low it/s
    strategy: str = "steps"
    steps: int = 50
