...
if [[ -n "$SAVE_SUBFOLDER" ]]; then
  echo "Configured save subfolder: $SAVE_SUBFOLDER"
fi
echo
echo "To activate manually in a new shell:"
echo "  source .venv_bertqa/bin/activate"
echo "To run train_by_steps with optional save subfolder:"
if [[ -n "$SAVE_SUBFOLDER" ]]; then
  echo "  ./run.sh 5 \"$SAVE_SUBFOLDER\""
else
  echo "  ./run.sh 5 [save_subfolder]"
fi
