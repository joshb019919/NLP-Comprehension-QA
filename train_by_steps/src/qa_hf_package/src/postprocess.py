...
from .metrics import normalize_answer_text
...
def postprocess_qa_predictions(
    ...
    for example_index, example in enumerate(examples):
        ...
        alias_list = example.get("aliases", []) or []
        answer_values = answers.get("text", []) if isinstance(answers, dict) else []
        if not answer_values and alias_list:
            answer_values = alias_list
        ...
        predictions.append({"id": example["id"], "prediction_text": final_text, "no_answer_probability": 0.0})
...
def select_best_alias_match(context: str, candidates: list[str]) -> tuple[int, str] | None:
    lowered_context = context.lower()
    best = None
    for candidate in candidates:
        if not candidate:
            continue
        idx = lowered_context.find(candidate.lower())
        if idx >= 0:
...
def build_alias_list(answer_value: str | None, aliases: list[str] | None) -> list[str]:
    values = []
    if answer_value:
    ...
    seen = set()
    for item in values:
        norm = normalize_answer_text(item)
